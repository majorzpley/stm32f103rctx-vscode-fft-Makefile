...
{
    uint16_t STMFLASH_BUF[STM_SECTOR_SIZE / 2]; // 一页<=2K字节
    uint32_t secpos;                            // 扇区地址
    uint16_t secoff;                            // 扇区内偏移地址
    uint16_t secremain;                         // 扇区剩余地址
    uint16_t i;
    uint32_t offaddr; // 去掉0x8000000地址
    ...
    while (1)
    {
        FlashRead(secpos * STM_SECTOR_SIZE + STM32_FLASH_BASE, STMFLASH_BUF, STM_SECTOR_SIZE / 2); // 读出整个扇区的内容
